...
with st.sidebar:
    ...
    *(SEG.IM es ignorado)*
    """)
    st.caption("v1.1 · Streamlit + pdftotext")
...
# ---------------------------------------------------------------------------
# Procesamiento — BCP y BBVA retornan lista directa
#                 INTERBANK retorna {"registros": [...], "alertas": [...]}
# ---------------------------------------------------------------------------

todos_los_registros = []
todas_las_alertas   = []   # solo se usa para INTERBANK
archivos_procesados = []
archivos_sin_datos  = []
    ...
    for archivo in archivos:
        ...
        try:
            if banco == "BCP":
                ...
                # Nuevo interbank retorna dict con "registros" y "alertas"
                resultado = interbank.procesar_pdf(archivo.name, ruta_tmp)
                registros = resultado["registros"]
                alertas   = resultado["alertas"]

            else:  # BBVA
                # bbva.procesar_pdf también retorna dict con "registros" y "alertas"
                resultado = bbva.procesar_pdf(archivo.name, ruta_tmp)
                registros = resultado["registros"]
                alertas   = resultado["alertas"]

            total_archivo = len(registros) + len(alertas)
...
if banco == "INTERBANK" and not df.empty:
...
if banco == "INTERBANK" and todas_las_alertas:
...
if banco == "INTERBANK" and todas_las_alertas:
    # 4 columnas: ITF, Gastos, Total, Alertas
...
with col1:
    ...
    if banco == "INTERBANK" and total_itf_con_alertas != total_itf:
        sub_itf += f" · con dudosos: S/ {total_itf_con_alertas:,.2f}"
    st.markdown(f"""
...
with col2:
    ...
    if banco == "INTERBANK" and total_gastos_con_alertas != total_gastos:
        sub_gst += f" · con dudosos: S/ {total_gastos_con_alertas:,.2f}"
    st.markdown(f"""
...
with col3:
    ...
    if banco == "INTERBANK" and (total_itf_con_alertas + total_gastos_con_alertas) != total_general:
        sub_tot += f" · con dudosos: S/ {total_itf_con_alertas + total_gastos_con_alertas:,.2f}"
    st.markdown(f"""
...
# ---------------------------------------------------------------------------
# Alertas INTERBANK — sección visible solo cuando hay dudosos
# ---------------------------------------------------------------------------
...
if banco == "INTERBANK" and todas_las_alertas:
    ...
    for a in todas_las_alertas:
        ...
        <div class="alerta-row">
            <strong>{a['Fecha']} · {a['Concepto detectado']} · {a['Símbolo']} {a['Monto']:,.2f}</strong><br>
            <span class="alerta-motivo">💡 {a['Motivo alerta']}</span><br>
            <code style="font-size:0.78rem; color:#78350f;">{a['Línea original']}</code>
        </div>
        """, unsafe_allow_html=True)
...
| GASTOS BANCARIOS | {} | S/ {:.2f} | Registrar un solo asiento mensual por comisiones y mantenimientos |
