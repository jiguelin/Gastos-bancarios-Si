...
from procesadores import bcp, interbank, bbva, pichincha, caja_huancayo, scotiabank
...
<div class="main-header">
    ...
    <span class="banco-badge">SCOTIABANK</span>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    banco = st.radio(
        "Selecciona el banco",
        options=["BCP", "INTERBANK", "BBVA", "PICHINCHA", "CAJA HUANCAYO", "SCOTIABANK"],
        index=0,
    )
    st.markdown("---")
    st.markdown("""
    ...
    **SCOTIABANK** *(Dólares)*
    - Impuesto a los Débitos (ITF)
    - Impuesto a los Créditos (ITF)
    - Mant Tarjet Empres Certificada
    - Comisión Tarjeta Joy Negocios
    - Intereses Acreedores (a favor)
    """)
    st.caption("v1.4 · Streamlit + pdfplumber + fitz")
...
with st.spinner(f"Procesando {len(archivos)} archivo(s)..."):
    for archivo in archivos:
        ...
        try:
            ...
            elif banco == "CAJA HUANCAYO":
                resultado = caja_huancayo.procesar_pdf(archivo.name, ruta_tmp)
            else:  # SCOTIABANK
                resultado = scotiabank.procesar_pdf(archivo.name, ruta_tmp)
...
if banco == "BCP" and not df.empty:
    ...
    for col in columnas:
        if col not in df.columns:
            df[col] = ""
    df = df[columnas]
elif banco == "SCOTIABANK" and not df.empty:
    columnas = scotiabank.get_columnas_ordenadas()
    for col in columnas:
        if col not in df.columns:
            df[col] = ""
    df = df[columnas]
...
# Intereses a favor (solo Pichincha por ahora)
df_intereses = df[df["Concepto"] == "INTERESES A FAVOR"].copy() if not df.empty else pd.DataFrame()
total_intereses = round(df_intereses["Monto"].sum(), 2) if not df_intereses.empty else 0.00
...
if total_intereses > 0:
    st.markdown(f"""
    <div class="metric-card" style="border-left-color:#16a34a; background:#f0fdf4; margin-top:0.5rem;">
        <div class="metric-label">🟢 Intereses a Favor</div>
        <div class="metric-value" style="color:#15803d;">{simbolo_display} {total_intereses:,.2f}</div>
        <div class="metric-sub">{len(df_intereses)} operación(es) · abono del banco al cliente</div>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    elif banco in ("PICHINCHA", "CAJA HUANCAYO", "SCOTIABANK") and not df.empty:
...
|---|---|---|---|
| IMPUESTO ITF | {len(df_itf)} | {simbolo_display} {total_itf:,.2f} | Registrar un solo asiento mensual por ITF |
| GASTOS BANCARIOS | {len(df_gastos)} | {simbolo_display} {total_gastos:,.2f} | Registrar un solo asiento mensual por comisiones y mantenimientos |"""
if total_intereses > 0:
    tabla_asiento += f"""
| INTERESES A FAVOR | {len(df_intereses)} | {simbolo_display} {total_intereses:,.2f} | Registrar como ingreso financiero a favor de la empresa |"""
st.markdown(tabla_asiento)
...
try:
    if banco == "BCP":
        ...
        # Pichincha, Caja Huancayo y Scotiabank usan el mismo formato que BBVA
        if banco == "PICHINCHA":
            modulo = pichincha
        elif banco == "CAJA HUANCAYO":
            modulo = caja_huancayo
        else:
            modulo = scotiabank
        excel_bytes = generar_excel_bbva(
            df, df_itf, df_gastos,
