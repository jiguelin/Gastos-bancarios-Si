...
def extraer_gastos(lineas: list, itf_max: float,
    ...
    # Detectar si el PDF completo tiene zona de "saldo por cobrar"
    # (hojas de cobranza donde TODOS los cargos son período anterior)
    tiene_zona_cobro = _detectar_zona_cobro(texto_completo)

    # Rastrear en qué "zona" estamos leyendo
    en_zona_cobro = False

    for l in lineas:
        l_stripped = l.strip()

        # Detectar entrada/salida de zona cobro
        if "SALDO CARGADO EN CUENTA" in l_stripped.upper():
            en_zona_cobro = True
        if "SALDO ANTERIOR" in l_stripped.upper() and "CARGADO" not in l_stripped.upper():
            en_zona_cobro = False
        ...
        if es_anterior:
            motivo = (
                f"Cargo de período anterior: '{desc_raw.strip()}'. "
                f"Este gasto corresponde a un mes previo al estado de cuenta. "
                f"Verificar si ya fue registrado en el período correspondiente "
                f"antes de contabilizarlo aquí."
            )
            alertas.append(_hacer_registro(
                archivo, anio, mes, ruc, cuenta, moneda, simbolo,
