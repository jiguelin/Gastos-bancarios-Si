...
def extraer_texto_pdf(ruta_pdf: str) -> str:
    """
    Usa pdfplumber.extract_words() para reconstruir el layout columnar
    del estado de cuenta Interbank preservando el alineado de columnas.
    Compatible con Streamlit Cloud (no requiere pdftotext instalado).
    """
    try:
        ...
        with pdfplumber.open(ruta_pdf) as pdf:
            for page in pdf.pages:
                ...
                # Agrupar palabras por posición Y (tolerancia 3pt)
                by_y = defaultdict(list)
                for w in words:
                    y = round(w["top"] / 3) * 3
                    by_y[y].append(w)

                # Reconstruir cada línea respetando espaciado horizontal
                for y in sorted(by_y.keys()):
                    ...
                    for w in ws:
                        gap = w["x0"] - prev_x1
                        spaces = max(1, int(gap / 5.5)) if prev_x1 > 0 else 0
                        line += " " * spaces + w["text"]
                        prev_x1 = w["x1"]
                    texto_total += line + "\n"
