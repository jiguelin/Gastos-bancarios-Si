...
# Umbral máximo para intereses (montos sobre esto son sospechosos)
UMBRAL_INTERESES = (500, 200)  # (soles, dólares)
...
PATRONES_IGNORAR = [
    "ORD ", "I/T-", "I/T ", "RETIRO",
    "CAPITALIZACION", "TRANSF CCE",
]

# Patrón para detectar intereses a favor (abono del banco al cliente)
PATRONES_INTERESES = ["INTERESES"]
...
def es_interes(descripcion: str) -> bool:
    """True si la línea es un abono de intereses del banco al cliente."""
    desc = normalizar(descripcion)
    return any(p in desc for p in PATRONES_INTERESES)
...
def _parsear_fisico(lineas: list, mes_estado: str):
    ...
    Retorna lista de dict con: fecha, descripcion, monto, es_abono, linea
    Captura tanto CARGOS (guión al final) como ABONOS (sin guión, ej: intereses).
    """
    movimientos = []
    TX_RE = re.compile(r'^(\d{2}/\d{2})\s+\d+\s+(.+)')
    # Monto abono: número sin guión que NO termina en CR (el CR es el saldo)
    ABONO_RE = re.compile(r'\b(\d{1,3}(?:,\d{3})*\.\d{2})(?![-CR])')
    ...
    for linea in lineas:
        ...
        # Detectar si es cargo (monto-) o abono (monto sin -)
        ...
        if cargos:
            ...
            # Buscar abono: número sin guión, sin CR
            linea_sin_saldo = re.sub(r'[\d,]+\.\d{2}CR\s*$', '', linea)
            abonos = ABONO_RE.findall(linea_sin_saldo)
            if not abonos:
                continue
            monto = limpiar_monto(abonos[-1])  # último número antes del saldo
            es_abono = True
...
def _parsear_fisico_campos(lineas: list):
    ...
    while i < len(lineas):
        ...
        movimientos.append({
            ...
            "monto":       monto,
            "es_abono":    False,  # campos separados solo captura cargos
...
def _parsear_sistema(lineas: list):
    ...
    for linea in lineas:
        ...
        movimientos.append({
            ...
            "descripcion": descripcion,
            "monto":       monto,
            "es_abono":    False,  # sistema solo captura DÉBITO aquí
            "linea":       l,
        })
...
def procesar_pdf(nombre_archivo: str, ruta_archivo: str) -> dict:
    ...
    for mov in movimientos:
        ...
        fecha    = mov["fecha"]
        linea    = mov["linea"]
        es_abono = mov.get("es_abono", False)

        # ── INTERESES A FAVOR ─────────────────────────────────────────────
        if es_abono and es_interes(desc):
            idx_umbral = 1 if moneda == "DOLARES" else 0
            umbral_int = UMBRAL_INTERESES[idx_umbral]
            if monto > umbral_int:
                motivo = (
                    f"Interés a favor de {simbolo} {monto:,.2f} supera el máximo esperado "
                    f"({simbolo} {umbral_int:,.2f}). Verificar en el estado de cuenta original."
                )
                alertas.append(_hacer_registro(
                    **campos_base, categoria="INTERESES A FAVOR",
                    concepto_detectado="INTERESES (sospechoso)",
